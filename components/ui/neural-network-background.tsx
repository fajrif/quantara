...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    // Mix colors based on CPPN output
    vec3 color1 = mix(uColorPrimary, uColorSecondary, buf[0].x);
    vec3 color2 = mix(uColorSecondary, uColorAccent, buf[0].y);
    vec3 finalColor = mix(color1, color2, buf[0].z * uIntensity);
...
const CPPNShaderMaterial = shaderMaterial(
    {
        ...
        uColorPrimary: new THREE.Vector3(0.12, 0.23, 0.54), // Default blue
        uColorSecondary: new THREE.Vector3(0.12, 0.16, 0.23), // Default slate
        uColorAccent: new THREE.Vector3(0.05, 0.65, 0.91), // Default sky
        uIntensity: 0.5,
    },
    vertexShader,
    fragmentShader
...
function hexToRgb(hex: string): THREE.Vector3 {
    ...
    return new THREE.Vector3(0.12, 0.23, 0.54) // Fallback
}
    ...
    useEffect(() => {
        if (materialRef.current && colorScheme) {
            if (colorScheme.primary) {
                materialRef.current.uColorPrimary = hexToRgb(colorScheme.primary)
            }
            if (colorScheme.secondary) {
                materialRef.current.uColorSecondary = hexToRgb(colorScheme.secondary)
            }
            if (colorScheme.accent) {
                materialRef.current.uColorAccent = hexToRgb(colorScheme.accent)
            }
